...
async function selectDevice(sdk, args) {
    ...
    if (!preferEmulator) {
        ...
        if (selectedDevice) {
            ...
            throw new errors_1.RunException(`No hardware devices found. Not attempting emulator because --device was specified.`, errors_1.ERR_NO_DEVICE);
        }
        else {
    ...
    try {
        ...
        if (e.code === errors_1.ERR_UNSUITABLE_API_INSTALLATION) {
            throw new errors_1.RunException('No targets devices/emulators available. Cannot create AVD because there is no suitable API installation. Use --sdk-info to reveal missing packages and other issues.', errors_1.ERR_NO_TARGET);
        }
    }
    throw new errors_1.RunException('No target devices/emulators available.', errors_1.ERR_NO_TARGET);
}
