...
async function getAVDINIs(sdk) {
    ...
    const contents = await utils_fs_1.readdir(avdHome);
...
async function createAVD(sdk, schematic) {
    ...
    await utils_fs_1.mkdirp(pathlib.join(avdHome, `${id}.avd`));
    await Promise.all([
        ini_1.writeINI(pathlib.join(avdHome, `${id}.ini`), ini),
        ini_1.writeINI(pathlib.join(avdHome, `${id}.avd`, 'config.ini'), configini),
    ]);
